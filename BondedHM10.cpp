...
BondedHM10* BondedHM10::_activeObject = NULL;
...
bool BondedHM10::disconnect()
{
  ...
  {
    // makeActive(); // Not needed here since the reset() function calls makeActive().
...
void BondedHM10::detectAndHandleConnection()
{
  ...
  else
  {
    ...
    else if ((_role == Role::Central)
                && _autoReconnectEnabled 
                  && ((millis() - _lastConnectAttemptTimestamp) >= _autoReconnectTimeout) 
...
bool BondedHM10::makeActive()
{
  if (_activeObject != this)
  {
    if (_activeObject)
    {
      _activeObject->makeInactive();
    }
...
bool BondedHM10::isActive()
{
  return (_activeObject == this);
}


bool BondedHM10::makeInactive()
{
  if (_activeObject == this)
  {
    _activeObject = NULL;
...
void BondedHM10::handleTransmissionTimerInterrupt()
{
  if (_activeObject)
  {
    if (_activeObject->_transmissionTimer > 0)
    {
      _activeObject->_transmissionTimer++;
    }

    if (_activeObject->_transmissionTimer >= TRANSMISSION_TIMER_DURATION)
    {
      _activeObject->stopTransmissionTimer();
    }
  }
}
